...
output "vm_internal_ips" {
  value = {
    for key, vm in yandex_compute_instance.vm :
    key => vm.network_interface.0.ip_address
  }
  description = "Internal IP addresses of created VMs"
}

output "disk_info" {
  value = {
    for key, disk in yandex_compute_disk.boot_disk :
    key => {
      id   = disk.id
      name = disk.name
      type = disk.type
  ...
  description = "Information about created boot disks"
}
